...
function numeroMasGrande(numeros) {
    ...
    // Tu código:
    return numeros.reduce((max, current) => Math.max(max, current));
}
...
function multiplicarArgumentos() {
    ...
    return arguments.length < 1 ? 0 : [].concat.apply([], arguments).reduce((a, b) => a * b);
}
...
function cuentoElementos(arreglo) {
    ...
    //Escribe tu código aquí
    return arreglo.filter((number) => number > 18).length;
}
